...
class CustomColorBlobLocatorProcessorImpl extends CustomColorBlobLocatorProcessor implements VisionProcessor
{
    ...
    public void onDrawFrame(Canvas canvas, int onscreenWidth, int onscreenHeight, float scaleBmpPxToCanvasPx, float scaleCanvasDensity, Object userContext)
    {
        ...
        for (Blob blob : blobs)
        {
            ...
            if (showCenter) {
                int lastPaintColor = contourPaint.getColor();
                contourPaint.setColor(Color.parseColor("white"));
                contourPaint.setStyle(Paint.Style.FILL);
                canvas.drawCircle(
                        (float) blob.getBoxFit().center.x * scaleBmpPxToCanvasPx,
                        (float) blob.getBoxFit().center.y * scaleBmpPxToCanvasPx, 12 * scaleCanvasDensity, contourPaint);
                contourPaint.setStyle(Paint.Style.STROKE);
                contourPaint.setColor(lastPaintColor);
            }
